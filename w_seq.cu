...
__host__
int get_next_word(char **memory, char **valids, int **char_count, int **char_count_atleast_flag){
    // Frequencies:
    ...
    int best_score = -1;
    int valid_word_index_iterator = 0;
    int value_to_return = -1;
    while ((*valids)[valid_word_index_iterator * 5] != '\0'){

        int is_valid = 1;
        for (int j = 0; j < 5; j++){ // iterate through the 5 letters
            int letter_as_number = (int)((*valids)[valid_word_index_iterator * 5 + j]) - 97;

            // count the number of occurences of the letter
            int occurances = 0;
            for (int k = 0; k < 5; k++){
                int letter_as_number_2 = (int)((*valids)[valid_word_index_iterator * 5 + k]) - 97;
                if (letter_as_number == letter_as_number_2)
                    occurances++;
            }

            if ((*char_count_atleast_flag)[letter_as_number] == 1 && occurances < (*char_count)[letter_as_number]){
                is_valid = 0;
                break;
            }
            else if ((*char_count_atleast_flag)[letter_as_number] == 0 && occurances != (*char_count)[letter_as_number]){
                is_valid = 0;
                break;
            }
        }
        ...
        if (is_valid == 0){
            valid_word_index_iterator += 1;
            continue;
        }

        // go through rows of memory 
        for (int j = 0; j < 5; j++){
            char letter_to_test = (*memory)[j * 6 + 0];
            ...
            int key_exists = 0;
            for (int k = 0; k < 5; k++){ 
                char letter_in_the_word_to_test = (*valids)[valid_word_index_iterator * 5 + k];
                char indicator = (*memory)[j * 6 + 1 + k];
        ...
        for (int j = 0; j < 26; j++)
            letter_done[j] = 0;
        for (int j = 0; j < 5; j++){
            char letter_to_compute_score = (*valids)[valid_word_index_iterator * 5 + j];
        ...
        if (score_cumm > best_score){
            best_score = score_cumm;
            value_to_return = valid_word_index_iterator;
        }
...
int solver_seq(char **valids){
    ...
    for (i = 0; i < 50; i++){ // 50 is a killswitch
        try_count++;
        ...
        if (i == 0)
            ...
            valid_word_index = get_next_word(&memory, valids, &char_count, &char_count_atleast_flag);
...
int main(){
    ...
    int valids_word_count = 0;
    textfile = fopen(WORDLE_WORD_LIST_FILE_NAME, "r");
    if(textfile == NULL)
        return 1;
    char tempArray[10];
    while(fgets(tempArray, 10, textfile)){
        valids[valids_word_count * 5 + 0] = tempArray[0];
        valids[valids_word_count * 5 + 1] = tempArray[1];
        valids[valids_word_count * 5 + 2] = tempArray[2];
        valids[valids_word_count * 5 + 3] = tempArray[3];
        valids[valids_word_count * 5 + 4] = tempArray[4];
        valids_word_count++;
    }
    valids[valids_word_count * 5] = '\0';
    fclose(textfile);
